%{
#include <stdio.h>
#include <stdlib.h>

int yylex(void);
void yyerror(const char *s);
extern FILE *yyin;
%}

%union {
    char* str;
    int val;
    int count;
}

%token DECLARE PREDICATE FUNCTION VARIABLE
%token EXIST ALL
%token TRUE FALSE
%token AND OR NOT IMPLIES EQUIV
%token BRACKET_OPEN BRACKET_CLOSE
%token SQUARE_BRACKET_OPEN SQUARE_BRACKET_CLOSE
%token COMMA COLON SEMICOLON

%token <val> INT 
%token <str> STRING
%type <count> term_list_opt term_list

%left OR
%left AND
%left IMPLIES
%left EQUIV
%right NOT

%start input

%%

input:
    input block | block
    ;

block:
    declarations formula SEMICOLON
    ;
    
declarations:
    declarations declaration | declaration
    ;

declaration:
    DECLARE PREDICATE STRING COLON INT {
        fprintf(stderr, "PAR: Declaration: Predicate -%s- Arity: %d\n", $3, $5);
        free($3);
    }
    | DECLARE FUNCTION STRING COLON INT {
        fprintf(stderr, "PAR: Declaration: Function -%s- Arity: %d\n", $3, $5);
        free($3);
    }
    | DECLARE VARIABLE STRING COLON STRING {
        fprintf(stderr, "PAR: Declaration: Variable -%s- Type: %s\n", $3, $5);
        free($3);
        free($5);
    }
;

formula:
    equiv_formula { fprintf(stderr, "PAR: Formula completed with Semicolon.\n"); }
;

equiv_formula:
      implies_formula
    | equiv_formula EQUIV implies_formula { fprintf(stderr,"PAR: Formula reduced - JUNCTOR: EQUIVALENT\n"); }
;

implies_formula:
      or_formula
    | implies_formula IMPLIES or_formula { fprintf(stderr,"PAR: Formula reduced - JUNCTOR: IMPLICATION\n"); }
;

or_formula:
      and_formula
    | or_formula OR and_formula { fprintf(stderr,"PAR: Formula reduced - JUNCTOR: OR\n"); }
;

and_formula:
      not_formula
    | and_formula AND not_formula { fprintf(stderr,"PAR: Formula reduced - JUNCTOR: AND\n"); }
;

not_formula:
      quant_or_atom
    | NOT not_formula { fprintf(stderr,"PAR: Formula reduced - JUNCTOR: NEGATION\n"); }
;

quant_or_atom:
      TRUE { fprintf(stderr,"PAR: Formula reduced - ATOM: TRUE\n"); }
    | FALSE { fprintf(stderr,"PAR: Formula reduced - ATOM: FALSE\n"); }
    | atom
    | ALL SQUARE_BRACKET_OPEN STRING SQUARE_BRACKET_CLOSE not_formula {
        fprintf(stderr,"PAR: Formula reduced - QUANTOR: ALL %s\n",$3); free($3); }
    | EXIST SQUARE_BRACKET_OPEN STRING SQUARE_BRACKET_CLOSE not_formula {
        fprintf(stderr,"PAR: Fromula reduced - QUANTOR: EXIST %s\n",$3); free($3); }
    | BRACKET_OPEN formula BRACKET_CLOSE
;

atom:
    STRING BRACKET_OPEN term_list_opt BRACKET_CLOSE {
        fprintf(stderr, "PAR: Formula reduced - ATOM: %s() - %d arguments\n", $1, $3);
        free($1);
    }
;

term_list_opt:
    term_list { $$ = $1; }
    | { $$ = 0; }
;

term_list:
    term { $$ = 1; }
    | term_list COMMA term { $$ = $1 + 1; }
;

term:
    STRING {
        fprintf(stderr, "PAR: TERM: Variable/Constant %s\n", $1);
        free($1);
    }
  | INT {
        fprintf(stderr, "PAR: TERM: Number: %d\n", $1);
    }
  | STRING BRACKET_OPEN term_list_opt BRACKET_CLOSE {
        fprintf(stderr, "PAR: TERM: Function %s() - %d arguments\n", $1, $3);
        free($1);
      }
;

%%

void yyerror(const char *s) {
    fprintf(stderr,"Parser error: %s\n", s);
}

int main(int argc, char *argv[]){
    if(argc < 2) {
        fprintf(stderr,"Usage: %s <inputfile>\n", argv[0]);
        return 1;
    }
    FILE *fp = fopen(argv[1], "r");
    if(!fp) {
        perror("fopen");
        return 1;
    }
    yyin = fp;

    return yyparse();
} 